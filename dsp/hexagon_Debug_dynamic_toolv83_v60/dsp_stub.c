...
static const char strings[5] = "fft\0";
static const uint16_t methodStrings[1] = {0};
static const uint16_t methodStringsArrays[1] = {0};
__QAIC_SLIM_EXPORT const Interface __QAIC_SLIM(dsp_slim) = {1,&(methodArrays[0]),0,0,&(methodStringsArrays [0]),methodStrings,strings};
#endif //_DSP_SLIM_H
#ifdef __cplusplus
extern "C" {
#endif

#ifndef _const_dsp_handle
#define _const_dsp_handle ((remote_handle)-1)
#endif //_const_dsp_handle

static void _dsp_pls_dtor(void* data) {
   remote_handle* ph = (remote_handle*)data;
   if(_const_dsp_handle != *ph) {
      (void)__QAIC_REMOTE(remote_handle_close)(*ph);
      *ph = _const_dsp_handle;
   }
}

static int _dsp_pls_ctor(void* ctx, void* data) {
   remote_handle* ph = (remote_handle*)data;
   *ph = _const_dsp_handle;
...
__QAIC_STUB_EXPORT remote_handle _dsp_handle(void) {
   remote_handle* ph;
   if(adsp_pls_add_lookup) {
      if(0 == adsp_pls_add_lookup((uint32_t)_dsp_handle, 0, sizeof(*ph),  _dsp_pls_ctor, "dsp",  _dsp_pls_dtor, (void**)&ph))  {
         return *ph;
      }
      return (remote_handle)-1;
   } else if(HAP_pls_add_lookup) {
      if(0 == HAP_pls_add_lookup((uint32_t)_dsp_handle, 0, sizeof(*ph),  _dsp_pls_ctor, "dsp",  _dsp_pls_dtor, (void**)&ph))  {
         return *ph;
      }
      return (remote_handle)-1;
   }
...
uint32_t _dsp_atomic_CompareAndExchange(uint32_t * volatile puDest, uint32_t uExchange, uint32_t uCompare);
...
__QAIC_STUB_EXPORT remote_handle _dsp_handle(void) {
   static remote_handle handle = _const_dsp_handle;
   if((remote_handle)-1 != handle) {
      return handle;
   } else {
      remote_handle tmp;
      int nErr = _dsp_pls_ctor("dsp", (void*)&tmp);
      if(nErr) {
         return (remote_handle)-1;
      }
      if(((remote_handle)-1 != handle) || ((remote_handle)-1 != (remote_handle)_dsp_atomic_CompareAndExchange((uint32_t*)&handle, (uint32_t)tmp, (uint32_t)-1))) {
         _dsp_pls_dtor(&tmp);
      }
      return handle;
   }
...
__QAIC_STUB_EXPORT int __QAIC_STUB(dsp_skel_invoke)(uint32_t _sc, remote_arg* _pra) __QAIC_STUB_ATTRIBUTE {
   return __QAIC_REMOTE(remote_handle_invoke)(_dsp_handle(), _sc, _pra);
}
...
__QAIC_STUB_EXPORT int __QAIC_STUB(dsp_fft)(void) __QAIC_STUB_ATTRIBUTE {
   uint32_t _mid = 0;
   return _stub_method(_dsp_handle(), _mid);
}
#ifdef __cplusplus
}
#endif
