...
#******************************************************************************
parser = argparse.ArgumentParser(prog='dsp_walkthrough.py', description=__doc__, formatter_class=RawTextHelpFormatter)
call_parser(parser)
...
def run_calc():
	...
	HEXAGON_SDK_ROOT=os.getenv('HEXAGON_SDK_ROOT')
	dsp_exe='{}/examples/common/mimosa/dsp/{}/ship/dsp'.format(HEXAGON_SDK_ROOT,variant)
	#libdsp='{}/examples/common/mimosa/dsp/{}/ship/libdsp.so'.format(HEXAGON_SDK_ROOT,variant)
	libdsp_skel='{}/examples/common/mimosa/dsp/{}/ship/libdsp_skel.so'.format(HEXAGON_SDK_ROOT,hex_variant)
	
	if _platform == "win32":
		clean_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/examples/common/mimosa/dsp tree_clean V='+variant+' '+Flag+' VERBOSE=1 || exit /b'
		build_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/examples/common/mimosa/dsp tree V='+variant+' '+Flag+' VERBOSE=1 || exit /b'
		clean_hexagon_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/examples/common/mimosa/dsp tree_clean V='+hex_variant+' VERBOSE=1 || exit /b'
		build_hexagon_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/examples/common/mimosa/dsp tree V='+hex_variant+' VERBOSE=1 || exit /b'
	else:
		clean_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/examples/common/mimosa/dsp tree_clean V='+variant+' '+Flag+' VERBOSE=1 || exit 1'
		build_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/examples/common/mimosa/dsp tree V='+variant+' '+Flag+' VERBOSE=1 || exit 1'
		clean_hexagon_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/examples/common/mimosa/dsp tree_clean V='+hex_variant+' VERBOSE=1 || exit 1'
		build_hexagon_variant = 'make -C ' + HEXAGON_SDK_ROOT + '/examples/common/mimosa/dsp tree V='+hex_variant+' VERBOSE=1 || exit 1'

	call_test_sig , APPS_DST, DSP_DST, LIB_DST, ADSP_LIB_PATH = get_DST_PARAMS(HEXAGON_SDK_ROOT)

	if not options.no_rebuild:
		print "---- Build dsp example for both Android and Hexagon ----"
		...
		print "---- Skip rebuilding dsp example for both Android and Hexagon ----"
	...
	print_and_run_push_cmd('adb wait-for-device push '+dsp_exe+' '+APPS_DST)
	print_and_run_cmd('adb wait-for-device shell chmod 777 '+APPS_DST+'/dsp')
	#print_and_run_cmd('adb wait-for-device push '+libdsp+' '+LIB_DST)

	print " ---- Push Hexagon Components ----"
	print_and_run_cmd('adb wait-for-device shell mkdir -p '+DSP_DST)
	print_and_run_cmd('adb wait-for-device push '+libdsp_skel+' '+DSP_DST)

	print "---- Direct dsp messages to logcat ---"
	print_and_run_cmd('adb wait-for-device shell "echo 0x1f > '+DSP_DST+'dsp.farf"')
	...
	print "---- Run dsp Example Locally on Android ----"
	print_and_run_cmd('adb wait-for-device shell ADSP_LIBRARY_PATH='+ADSP_LIB_PATH+' '+APPS_DST+'/dsp 1 1000')
	print("---- Run dsp Example on " +Flag[:4] +" ----")
	print_and_run_cmd('adb wait-for-device shell ADSP_LIBRARY_PATH='+ADSP_LIB_PATH+' '+APPS_DST+'/dsp 0 1000')
